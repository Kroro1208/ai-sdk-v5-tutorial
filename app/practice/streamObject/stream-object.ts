  ...
  try {
    ...
    const result = await streamObject({
      ...
      onFinish({ object, usage }) {
        console.log("\n=== ストリーミング完了 ===");
        console.log("最終オブジェクト:", JSON.stringify(object, null, 2));
        console.log(`入力トークン: ${usage.inputTokens}`);
        console.log(`出力トークン: ${usage.outputTokens}`);
        console.log(`合計トークン: ${usage.totalTokens}`);
      },
    });
